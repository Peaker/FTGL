...
import Distribution.Simple.LocalBuildInfo (LocalBuildInfo, localPkgDescr, withPrograms, buildDir, absoluteInstallDirs)
...
myBuildHook pkg_descr local_bld_info _user_hooks _bld_flags =
    ...
    let lib = unjust "library of pkg_descr" (library pkg_descr)
        ...
        gcc = unjust "lookupProgram gcc" (lookupProgram (simpleProgram "gcc") progs)
...
compileCxx gcc opts incls out_path cxx_src =
    ...
    when do_it $ createDirectoryIfMissing True (dropFileName out_file) >>
                 runProgram verbose gcc (addIncludes ++ opts' ++ out)
    return out_file
